...
void wifi_connet_send_temp(DATA_T data)
{
    if (wifi_connect())
    {
...
void dht_task(void *args)
{
    ...
    while (1)
    {
        ...
        esp_err_t res = dht_read_data(dht_sensor, gpio_pin, &data.hum, &data.temp);

        if (res == ESP_OK)
        {
            ESP_LOGI("main_task", "Humidity=%d.%d%%, Temperature=%d.%d°C",
                     data.hum / 10, data.hum % 10, data.temp / 10, data.temp % 10);
            vTaskDelay(pdMS_TO_TICKS(1000));
            wifi_connet_send_temp(data);
        }
    }
}
